...
pub(crate) struct ManagedTorrentOptions {
    ...
    // Atlas patch: in-order (sequential) piece download. Runtime-toggleable so the UI
    // can flip it without re-adding the torrent. Default false = normal first/last/rarest.
...
impl ManagedTorrent {
    ...
    // Atlas patch: read/toggle sequential (in-order) piece download at runtime.
    // Takes effect on the next piece acquisition; no re-add needed.
    pub fn is_sequential(&self) -> bool {
        self.shared.options.sequential()
    }
    ...
    // Atlas patch: drop the torrent into a fresh Initializing state with
    // previously_errored=true, which forces a full on-disk re-hash of every piece
    // (bypassing fastresume) — the qBittorrent "force recheck". Pauses first if Live.
    // The caller then start()s it again; see Session::force_recheck.
    pub(crate) fn force_recheck_reinit(self: &Arc<Self>) -> anyhow::Result<()> {
        let mut g = self.locked.write();
        // Leave Live (this cancels the live torrent); we re-open storage from disk below.
