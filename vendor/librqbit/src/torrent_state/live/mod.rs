    ...
    fn acquire_next_piece(&self) -> crate::Result<Option<ValidPieceIndex>> {
        ...
        let result = self
            ...
            .with_live_mut(self.addr, "acquire_next_piece", |live| {
                ...
                // Atlas patch: in sequential mode, after active-stream pieces, prefer
                // pieces in ascending index order so the file fills front-to-back. The
                // acquire loop skips already-have/in-flight/not-offered pieces, so this
                // lazily resolves to the lowest needed piece the peer can serve.
                let lengths = &self.state.lengths;
                let priority_pieces: Box<dyn Iterator<Item = ValidPieceIndex>> =
                    if self.state.shared.options.sequential() {
                        Box::new(self.state.streams.iter_next_pieces(lengths).chain(
                            (0..lengths.total_pieces())
                                .filter_map(move |i| lengths.validate_piece_index(i)),
                        ))
                    } else {
                        Box::new(self.state.streams.iter_next_pieces(lengths))
                    };
