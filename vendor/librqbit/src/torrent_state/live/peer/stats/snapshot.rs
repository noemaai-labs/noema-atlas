...
pub struct PeerStats {
    ...
    /// Atlas patch: the remote peer's BitTorrent peer_id (20 raw bytes), present
    /// only while the peer is Live. Upstream never exposed this; the UI decodes the
    /// client name from it (Azureus/Shadow peer_id conventions).
...
impl From<&Peer> for PeerStats {
    fn from(peer: &Peer) -> Self {
        ...
        Self {
            ...
            peer_id: match state {
                PeerState::Live(l) => Some(l.peer_id.0),
                _ => None,
            },
        }
    }
