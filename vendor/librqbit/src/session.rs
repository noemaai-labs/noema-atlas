...
pub struct AddTorrentOptions {
    ...
    /// Atlas patch: download pieces in order (sequential). Default false.
    #[serde(default)]
    pub sequential: bool,
}
...
impl Session {
        ...
        let (managed_torrent, metadata) = {
            ...
            let minfo = Arc::new(ManagedTorrentShared {
                ...
                options: ManagedTorrentOptions {
                    ...
                    sequential: std::sync::atomic::AtomicBool::new(opts.sequential),
                    #[cfg(feature = "disable-upload")]
    ...
    // Atlas patch: force a full re-hash of a torrent's on-disk files (qBittorrent
    // "force recheck"). Re-initializes with previously_errored=true so fastresume is
    // bypassed, then resumes — the initializer re-validates every piece against the
    // torrent's hashes and rebuilds the have/needed sets from what's actually on disk.
    pub async fn force_recheck(
        self: &Arc<Self>,
        handle: &ManagedTorrentHandle,
    ) -> anyhow::Result<()> {
        handle.force_recheck_reinit()?;
        let peer_rx = self.make_peer_rx_managed_torrent(handle, true);
        handle.start(peer_rx, false)?;
        self.try_update_persistence_metadata(handle).await;
        Ok(())
