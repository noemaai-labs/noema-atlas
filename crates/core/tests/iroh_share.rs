...
async fn seed_and_fetch(provider: &IrohNode, fetcher: &IrohNode, src: &Path, dest: &Path) -> bool {
    ...
    fetcher
        .fetch_from_providers(&blake3, &[ticket], dest, None, None)
        .await
        .is_ok()
}
...
async fn unseed_and_disconnect_stops_serving() {
    ...
    assert!(fetcher
        .fetch_from_providers(&blake3, &[ticket.clone()], &dest1, None, None)
        .await
        .is_ok());
    provider
    ...
    let fetched = fetcher
        .fetch_from_providers(&blake3, &[ticket], &dest2, None, None)
        .await;
    assert!(
        fetched.is_err(),
...
async fn hard_disconnects_a_peer_mid_transfer() {
    ...
    let d = dest.clone();
    let fetch =
        tokio::spawn(async move { f.fetch_from_providers(&b3, &[t], &d, None, None).await });
